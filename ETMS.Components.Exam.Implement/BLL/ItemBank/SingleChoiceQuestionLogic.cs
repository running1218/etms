// File:    SingleChoiceQuestionLogic.cs
// Author:  Administrator
// Created: 2011年12月16日 10:27:23
// Purpose: Definition of Class SingleChoiceQuestionLogic
...
namespace ETMS.Components.Exam.Implement.BLL.ItemBank
{
    ...
    /// 单选题逻辑实现
    ///</summary>
    internal class SingleChoiceQuestionLogic : IMessageSourceAware, IInitializingObject, IQuestionServiceLogic
    {
        private static string Err_SingleChoiceQuestion_Invalid_Type = "ItemBank.SingleChoiceQuestion.Invalid.Type";
        private static string Err_SingleChoiceQuestion_Invalid_Data = "ItemBank.SingleChoiceQuestion.Invalid.Data";
        private static string Err_SingleChoiceQuestion_Not_Found = "ItemBank.SingleChoiceQuestion.Not.Found";
        ...
        public Guid AddQuestion(QuestionBase question)
        {
            ...
            SingleChoiceQuestion MyQuestion = (SingleChoiceQuestion)question;
        ...
        private int ValidAnswer(SingleChoiceQuestion MyQuestion, out string sErrMsg)
        {
            sErrMsg = "";
            ...
            //if (MyQuestion.Answer.AnswerOption.)
            ...
            System.Guid QuestionID, out string sErrMsg)
        {
            ...
            var LstTmp2 = (from item in LstOptions
                           where item.QuestionID != QuestionID && item.OptionID != Guid.Empty
        ...
        private void ThrowNotValid(QuestionBase question, System.Guid questionID)
        {
            ...
            {
                throw new ETMS.AppContext.BusinessException(Err_SingleChoiceQuestion_Invalid_Data, new Exception("试题数据不完整"));
            }
            //检查类型
            if (!(question is SingleChoiceQuestion))
            {
                throw new ETMS.AppContext.BusinessException(Err_SingleChoiceQuestion_Invalid_Type, new Exception("试题的类型非单选题"));
            }
            SingleChoiceQuestion MyQuestion = (SingleChoiceQuestion)question;
            //检查试题选项
            string sErrMsg = "";
            ...
            if (nValid != 0)
            {
                throw new ETMS.AppContext.BusinessException(Err_SingleChoiceQuestion_Invalid_Data, new Exception(sErrMsg));
            }
            //验证答案是否合法
        ...
        public void Update(Guid questionID, QuestionBase newQuestion)
        {
            ...
            //得到原试题
            SingleChoiceQuestion MyOldQuestion = (SingleChoiceQuestion)this.GetByID(questionID);
            if (MyOldQuestion == null || MyOldQuestion.CommonQuestion == null
                || MyOldQuestion.CommonQuestion.Question == null)
            {
                throw new ETMS.AppContext.BusinessException(Err_SingleChoiceQuestion_Not_Found, new Exception("试题已不存在，无法更新"));
            }

            SingleChoiceQuestion MyQuestion = (SingleChoiceQuestion)newQuestion;
        ...
        public QuestionBase GetByID(Guid questionID)
        {
            ...
            if (CommonQuestion == null || CommonQuestion.Question == null)
                return null;
            //检查试题类型是否正确
            if (CommonQuestion.Question.QuestionType != QuestionType.SingleChoice)
            {
                throw new ETMS.AppContext.BusinessException(Err_SingleChoiceQuestion_Invalid_Type, new Exception("试题类型非单选题"));
            }
            ...
            if (OptionAnswer != null)
            {
                ...
                {
                    SingleChoiceAnswer oAnswer = new SingleChoiceAnswer(sAnswer, LstAnswerOptions[0]);
