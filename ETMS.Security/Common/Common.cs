...
namespace ETMS.Security
{
    ...
    public class Common
    {
        ...
        internal static string C_TICKET_COOKIE = "HTicket_";
        /// <summary>
        ...
        /// <param name="appSignID"></param>
        /// <returns></returns>
        public static XmlDocument GenerateSignInInfo(ETMS.Components.Basic.API.Entity.Security.IUser userInfo, bool bDontSaveUserID, bool bAutoSignIn, string appSignID)
        {
            ...
            XmlHelper.AppendNode(xmlDoc.DocumentElement, "SSID", appSignID);
            ...
            #region 开启自动登录时，默认时间策略：DateTime.MaxValue,以保证用户下次不需要再次登录！
            if (bAutoSignIn)
                dtExpireTime = DateTime.MaxValue;
            #endregion
