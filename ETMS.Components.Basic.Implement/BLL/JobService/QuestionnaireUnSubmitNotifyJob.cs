...
{
    public class QuestionnaireUnSubmitNotifyJob : IJobService
    {
        ...
        public QuestionnaireUnSubmitNotifyJob(int queryID, string resourceTypeCode)
        {
        ...
        public int DoJob()
        {
            string objectName = string.Format("QuestionnaireUnSubmitNotifyJob_{0}", DateTime.Now.ToString("yyyyMMddHHmmss"));
            ...
            if (totalRecords == 0)
            {
                ...
                try
                {
                    System.Data.DataTable dt = new Poll_UserResourceQueryResultDataAccess().GetUnSubmitPaperUserByQuerID(queryID, resourceTypeCode);
                    QuestionnaireUnSubmitSendNotifyJob.Notify(dt);
                    if (this.Logger != null && this.Logger.IsDebug)
                    {
                ...
                catch (Exception ex)
                {
                    ...
                    throw new ETMS.AppContext.BusinessException("QuestionnaireUnSubmitNotifyJob.Failed", new object[] { this.queryID }, ex);
                }
                finally
                {
        ...
        public OES.Logger.ILog Logger
        {
            get
            {
                if (m_Logger == null)
                {
                    m_Logger = new OESLoggerAdapter(string.Format("调查问卷未提交_{0}", this.queryID));
                }
                return m_Logger;
            }
            set
