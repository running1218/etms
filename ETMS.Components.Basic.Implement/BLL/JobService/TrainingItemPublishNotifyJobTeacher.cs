...
{
    /// <summary>
    /// 培训项目发布提醒教师Job（页面事件触发）
    /// </summary>
    public class TrainingItemPublishNotifyJobTeacher : IJobService
    {
           private Log_SystemInfoLogic SystemInfoLogic = new Log_SystemInfoLogic();

        private Tr_ItemCourseTeacherLogic ItemCourseTeacherLogic = new Tr_ItemCourseTeacherLogic();
        /// <summary>
        /// 培训项目ID
        /// </summary>
        public Guid TrainingItemID { get; set; }

        public TrainingItemPublishNotifyJobTeacher(Guid trainingItemID)
        {
        ...
        public int DoJob()
        {
            string objectName = string.Format("TrainingItemPublishNotifyJobTeacher_{0}", this.TrainingItemID.ToString("n"));
            ...
            if (totalRecords == 0)
            {
                ...
                try
                {
                    System.Data.DataTable dt = ItemCourseTeacherLogic.GetItemCourseTeacherList(this.TrainingItemID);
                    TrainingItemBeginNotifyJobTeacher.Notify(dt);
                    if (this.Logger != null && this.Logger.IsDebug)
                    {
