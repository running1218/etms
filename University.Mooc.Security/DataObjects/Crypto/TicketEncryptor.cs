...
namespace University.Mooc.Security
{
    ...
    public class TicketEncryption : ITicketEncryption
    {
        ...
        public byte[] EncryptTicket(ITicket ticket, object oParam)
        {
            ...
            if (strKeyInfo.IndexOf("<Modulus>") != -1)//基于RSAKeyValue方式配置
            {
                rsa = new RSACryptoServiceProvider();
                rsa.FromXmlString(strKeyInfo);//加载RSAKey
            }
            else//基于RSAKeyContainerName方式配置
            {
                CspParameters cspParams = new CspParameters();
                cspParams.Flags = CspProviderFlags.UseMachineKeyStore;
                cspParams.KeyContainerName = strKeyInfo;
