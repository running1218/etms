...
namespace University.Mooc.Security
{
    ...
    public sealed class PassportManager
    {
        ...
        public static bool IsOtherLoginIn(string sessionID, int userID)
        {
            var source = new PassportAuthenticateLogic().GetPassportAccessLog(userID);

            if (null != source && source.Rows.Count > 0)
            {
                if (sessionID.Equals(source.Rows[0]["LastLoginIP"].ToString(), StringComparison.InvariantCultureIgnoreCase) && source.Rows[0]["LoginStatus"].ToString().ToLower() == "true")
                {
                    return false;
                }
                else
