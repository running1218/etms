...
function loadLivings() {
    var params = { "CourseID": CourseID, "TrainingItemCourseID": TrainingItemCourseID, "UserID": userid };
    common.call(AppPath + "/MyTrain/GetUserCourseLivings", params, 'get', function (data) {
        if (data.Status == true) {
            if (data.Data.length > 0) {
                var template = Handlebars.compile($("#tmpl_CourseDetailStudy").html());
                var renderHTML = template(data);
...
function toLivingRoom(livingID) {
...
$(function () {
    ...
    //预设直播课程当前资源ID为项目课程ID
    ResourceID = TrainingItemCourseID;
    loadCourseDetail(); //课程信息
    loadLivings(); //直播目录表+学习状态
    loadQuestionList();//加载问答列表
    loadNoteList();//笔记列表
