...
function courseExercise(data) {
    // console.log(data)
    if (data.Status) {
        commitStatus = data.Data.UserTestCount < data.Data.TestCount ? 0 : 1;
        if (commitStatus) {
            MOrCFun();
        }
        ...
        testTime = data.Data.LimitTime;//倒计时时间
        //console.log(_paper)     
...
//倒计时
function testTimerFun() {
    var hour = 0;
    var minutes = 0;
    var seconds = 0;
    if (testTime <= 0) {
        $(".countTimer").html("00:00:00");
        //提交试卷  
        getParameter();
        SaveOrCommitPaper();
        ...
        common.call(AppPath+'/Paper/PostSubmitStudentPaper', saveParams, 'post', submitTestCaution, error);

    } else {
        var timer = setInterval(function () {
            hour = Math.floor(testTime / 3600);
            minutes = Math.floor((testTime / 60 - hour * 60));
            seconds = Math.floor(testTime - minutes * 60 - hour * 60 * 60);
            if (hour <= 9) { hour = "0" + hour; }
            if (minutes <= 9) { minutes = "0" + minutes; }
            if (seconds <= 9) { seconds = "0" + seconds; }
            $(".countTimer").html(hour + ":" + minutes + ":" + seconds);
            testTime--;
            if (testTime <= 0) {
                clearInterval(timer);
                //提交试卷  
...
//到达答题时间上线，提交提醒
function submitTestCaution(data) {
    if (data.Status) {
        swal("答题时间已到，提交成功！", "", "success");
