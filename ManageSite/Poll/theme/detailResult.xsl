...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
      ...
      <xsl:choose>
        <xsl:when test="OtherLength = 0 ">
          <li>
            ...
            <xsl:if test="IsChecked = 1">
              <input type="radio" class="inputCircle"  checked="checked"  name="{TitleID}" TitleID="{TitleID}" OptionID="{OptionID}" TitleType="1" IsOther="0"/>
            </xsl:if>
            <xsl:if test="normalize-space(IsChecked)=''">
              <input type="radio" class="inputCircle"  name="{TitleID}" TitleID="{TitleID}" OptionID="{OptionID}" TitleType="1" IsOther="0"/>
            </xsl:if>
        ...
        <xsl:otherwise>
          <li>
            ...
            <input checked="checked" type="radio" class="inputCircle"  name="{TitleID}" id="rboption_{OptionID}"  TitleID="{TitleID}" OptionID="{OptionID}"  TitleType="1" IsOther="1"  />
            <xsl:value-of select="OptionName"/>
            <input type="text" class="inputbox_300" value="{OtherText}" maxlength="100" id="txtoption_{OptionID}" readonly="true"  />(100字以内)
          </li>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:for-each>
      ...
      <xsl:choose>
        <xsl:when test="OtherLength = 0 ">
          <li>
            ...
            <xsl:if test="IsChecked = 1">
              <input type="checkbox" checked="checked" class="inputCircle" name="{TitleID}" TitleID="{TitleID}" OptionID="{OptionID}" TitleType="2" IsOther="0"/>
            </xsl:if>
            <xsl:if test="normalize-space(IsChecked)=''">
              <input type="checkbox" class="inputCircle" name="{TitleID}" TitleID="{TitleID}" OptionID="{OptionID}" TitleType="2" IsOther="0"/>
            </xsl:if>
            <!--</xsl:if>-->
        ...
        <xsl:otherwise>
          <li>
            ...
            <xsl:value-of select="OptionName"/>
            <input type="text" value="{OtherText}" class="inputbox_300" maxlength="100" id="txtoption_{OptionID}" readonly="true"   />(100字以内)
          </li>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:for-each>
  ...
  <xsl:template name="Text">
    <li>
      <textarea class="inputbox_area440" TitleID="{TitleID}" TitleType="4" onchange="save(this);">
