...
using System.Web;
public partial class Reporting_OnlineStudyCourseDetail : System.Web.UI.Page
{
    ...
    private System.Collections.IList PageDataSource(int pageIndex, int pageSize, out int totalRecordCount)
    {
        ...
        DataTable dt = onLineLogic.GetOnlineStudyPageList(GetOnlineItemCourseWhere(), ETMS.AppContext.UserContext.Current.OrganizationID, pageIndex, pageSize, out totalRecordCount);
    ...
    private OnlineItemCourse GetOnlineItemCourseWhere()
    {
        ...
        itemCourse.ItemCourseTeachModelID = -1;
        itemCourse.Status = ddlUserStatus.SelectedValue.ToInt();
        return itemCourse;
    }
    ...
    private void Export()
    {
        ...
        //导出
        string xmlTemplatePath = Server.MapPath(@"ExportXML/OnlineStudyCourseDetail.xml");
        int fileNameCount = 1;//防止文件同名
        string fileName = "在线学习情况监控" + fileNameCount;
        ...
        string pathFileName = Server.MapPath("~/Temp/" + savePathFilesName + "/" + fileNameCount + fileName + strDateTime + ".xls");
         dt = new OnlineStudyLogic().GetOnlineStudyPageList(GetOnlineItemCourseWhere(), ETMS.AppContext.UserContext.Current.OrganizationID, 1, sheetSize, out totalRecordCount);
        ...
        if (totalRecordCount > sheetSize)
        {
            for (int i = 1; i <= totalRecordCount / sheetSize; i++)
            {
                ...
                int pageIndex = i + 1;
                dt = new OnlineStudyLogic().GetOnlineStudyPageList(GetOnlineItemCourseWhere(), ETMS.AppContext.UserContext.Current.OrganizationID,pageIndex,sheetSize, out totalRecordCount);
                ExportExcel(xmlTemplatePath, fileName, dt, pathFileName);
            }
        }
        ...
        //下载
        //FileDownLoadUtility.ExportFile(Server.MapPath("~/Temp/" + savePathFilesName + ".zip"));
        //FileDownLoadUtility.DownFileStream(savePathFilesName + ".zip", downDictory);
