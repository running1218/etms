<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPages/MPagePop.Master" AutoEventWireup="true" CodeFile="CourseWareSort.aspx.cs" Inherits="TraningImplement_ProjectCourseResource_CourseWareSort" %>
        ...
        $(document).ready(function () {
            ...
            $("#inpSave").click(function () {
                ...
                for (var i = 0; i < CourseList.length; i++) {
                    CourseWareIDs += CourseList.options[i].value + ",";
                }
                if (CourseWareIDs == "") {
                    popAlertMsg('没有可排序的课件。', '提示');
                    return;
                }

                $.post("Controls/CourseWareSort.ashx", {
                    CourseWareIDName: CourseWareIDs
                }, function (data) {
                    if (data == "success") {
                        popSuccessMsg('课件排序成功。', '提示', function () { window.parent.location = window.parent.location });
                    } else {
                        popFailedMsg('课件排序失败。原因如下：' + data, '提示');
                    }
                });
            });
