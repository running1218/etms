<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/MasterPages/MPagePop.Master" CodeFile="SetCourseResourceSort.aspx.cs" Inherits="Resource_CourseManage_SetCourseResourceSort" %>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#inpSave").click(function () {
                var ItemCourseContentIDs = "";
                for (var i = 0; i < CourseList.length; i++) {
                    ItemCourseContentIDs += CourseList.options[i].value + ",";
                }
                if (ItemCourseContentIDs == "") {
                    popAlertMsg('没有可排序的资源。', '提示');
                    return;
                }

                $.post("Controls/ItemCourseResourceSort.ashx", {
                    ItemCourseContentIDs: ItemCourseContentIDs
                }, function (data) {
                    if (data == "success") {
                        popSuccessMsg('课程资源排序成功。', '提示',
                        function () {
                            var url = window.parent.location.href;
                            if (url.indexOf('add') > 0) {
                                url = url.replace(/&oper=add/, '');
                            }
                            window.parent.location.href = url;
                        });
                    } else {
                        popFailedMsg('课程资源排序失败。原因如下：' + data, '提示');
                    }
                });
            });
