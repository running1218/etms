<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/MasterPages/MPagePop.Master" CodeFile="BannerSort.aspx.cs" Inherits="SiteManage_BannerManager_BannerSort" %>
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            var BannerList = $("#<%= lbBannerSort.ClientID %>")[0];
            $("#inpSave").click(function () {
                var ItemBannerIDs = "";
                for (var i = 0; i < BannerList.length; i++) {
                    ItemBannerIDs += BannerList.options[i].value + ",";
                }
                if (ItemBannerIDs == "") {
                    popAlertMsg('没有可排序的Banner。', '提示');
                    return;
                }

                $.post("../Controls/BannerSort.ashx", {
                    ItemBannerIDs: ItemBannerIDs
                }, function (data) {
                    if (data == "success") {
                        popSuccessMsg('排序成功。', '提示',
                            function () { window.parent.location = window.parent.location });
                    } else {
                        popFailedMsg('排序失败。原因如下：' + data, '提示');
                    }
                });
            });
            ...
            if (BannerList.length > 0) {
                BannerList.selectedIndex = 0;
            }

            var x = null;
            //将选中item向上
            function upListItem() {
                var selIndex = BannerList.selectedIndex;
                if (selIndex < 0) {
                ...
                var selValue = BannerList.options[selIndex].value;
                var selText = BannerList.options[selIndex].text;
                BannerList.options[selIndex].value = BannerList.options[selIndex - 1].value;
                BannerList.options[selIndex].text = BannerList.options[selIndex - 1].text;
                BannerList.options[selIndex - 1].value = selValue;
                BannerList.options[selIndex - 1].text = selText;
                BannerList.selectedIndex = selIndex - 1;
                if (selIndex + 1 > 0) {
            ...
            function downListItem() {
                ...
                if (selIndex == BannerList.options.length - 1) {
                    if (x != null) { clearTimeout(x); }
                    return;
                }
                var selValue = BannerList.options[selIndex].value;
                ...
                BannerList.options[selIndex + 1].text = selText;
                BannerList.selectedIndex = selIndex + 1;
                if (selIndex + 1 < BannerList.options.length - 1) {
        ...
        <table class="GridviewGray GridviewGrayboder">
            ...
            <tr>
                <td style="text-align: center;">
                    <select id="lbBannerSort" runat="server" class="list-box" size="25" style="width: 300px;">
                    </select>
