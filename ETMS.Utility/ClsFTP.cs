...
namespace ETMS.Utility
{
    ...
    public class ClsFTP
    {
        ...
        /// <param name="uri">FTP地址</param>
        /// <param name="FtpMathod">操作命令</param>
        private FtpWebResponse Open(Uri uri, string phyPath, string FtpMathod)
        {
            try
            {
                Request = (FtpWebRequest)WebRequest.Create(uri);
        ...
        public bool UploadFile(Stream fileStream, string RemoteFileName)
        {
            return UploadFile(fileStream, RemoteFileName, true);
        }
        ...
        /// <param name="RemoteFileName">要在FTP服务器上面保存文件名</param>
        /// <param name="OverWriteRemoteFile">是否覆盖远程服务器上面同名的文件</param>
        public bool UploadFile(Stream fileStream, string RemoteFileName, bool OverWriteRemoteFile)
        {
            try
            {
                if (!IsValidFileChars(RemoteFileName))
                ...
                if (fileStream.Length < (int.MaxValue - 1))
                {
                    byte[] bt = new byte[fileStream.Length];
                    fileStream.Read(bt, 0, (Int32)fileStream.Length);   //注意，因为Int32的最大限制，最大上传文件只能是大约2G多一点
                    fileStream.Close();
                    return UploadFile(bt, RemoteFileName, OverWriteRemoteFile);
                }
                else
                {
                    throw new Exception("上传文件大于上传限制！");
                }               
            }
            catch (Exception ep)
            {
        ...
        public bool UploadFile(byte[] FileBytes, string RemoteFileName, bool OverWriteRemoteFile)
        {
            try
            {
                ...
                //判断是否存在相应的目录
                DeclareOrCreateDirectory(RemoteFileName);
        ...
        private void DeclareOrCreateDirectory(string remoteFilePath)
        { 
            string[] paths = remoteFilePath.Split(new char[]{'/'});
            string directorys = string.Empty;

            for (int i = 0; i < paths.Length - 1; i++)
            {
                directorys = directorys != string.Empty ? string.Join("/", directorys, paths[i]) : paths[i];
    
                if (!IsExistDirectory(directorys))
                {
        ...
        /// 列出FTP服务器上面当前目录的所有文件和目录
        /// </summary>
        public FileStruct[] ListFilesAndDirectories(string phyPath)
        {
            Response = Open(this.Uri, phyPath, WebRequestMethods.Ftp.ListDirectoryDetails);
            StreamReader stream = new StreamReader(Response.GetResponseStream(), Encoding.Default);
            string Datastring = stream.ReadToEnd();
            FileStruct[] list = GetList(Datastring);
            return list;
        ...
        public FileStruct[] ListDirectories(string line)
        {
            FileStruct[] listAll = GetList(line);
            List<FileStruct> listDirectory = new List<FileStruct>();
            foreach (FileStruct file in listAll)
            {
                if (file.IsDirectory)
        ...
        public bool IsExistDirectory(string phyPath)
        {
            string parentPath = phyPath.Substring(0, phyPath.LastIndexOf('/') == -1 ? 0 : phyPath.LastIndexOf('/'));
            string directionary = phyPath.Substring(phyPath.LastIndexOf('/') == -1 ? 0 : phyPath.LastIndexOf('/') + 1);

            Uri uri = new Uri(string.Format("{0}/{1}", this.Uri.ToString(), parentPath));
            using (WebResponse response = Open(uri, WebRequestMethods.Ftp.ListDirectoryDetails))
            {
                using (StreamReader reader = new StreamReader(response.GetResponseStream()))
                {
                    string line = reader.ReadToEnd();
                    FileStruct[] listDir = ListDirectories(line);
                    foreach (FileStruct dir in listDir)
                    {
                        if (string.Compare(dir.Name, directionary, true) == 0)
                        {
                            return true;
                        }
                    }
