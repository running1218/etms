<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="AcUpFile.ascx.cs" Inherits="ETMS.Studying.Controls.AcUpFile" %>
...
<script type="text/javascript">
    var uploader;
    function editfiletype(filetypestring){
        //debugger;
        if(uploader!=undefined && uploader!=null){
            uploader.destroy();
        }
        createupload(filetypestring);
        alert(filetypestring);
    }

    function createupload(filetypestring){
        ...
        filters: {
            ...
            mime_types: [
                { title: "文件类型", extensions: ""+filetypestring+"" },
                ...
                FileUploaded: function (up, file, info) {            
                    if(info.response.substr(0,1)=='{')
                    {
                        ...
                        if(typeof(<%=CallBack%>) == 'function'){
                            <%=CallBack%>(json[0].FileName,json[0].FullUrl,json[0].FileSize,json[0].FileOldName,json[0].FileNames);                    
                        }
                    }                
                },    
