...
{
    public partial class CourseLivingView : System.Web.UI.Page
    {
        ...
        private void InitialControlers()
        {
            ...
            //绑定教学资源
            List<Res_Living> dtResource = livingLogic.GetLivingByCourseID(CourseID);//new Res_ContentLogic().GetPagedList(1, int.MaxValue - 1, " RCCR.[Sort] ASC ", " AND RCCR.[Status]=1 ", CourseID, out totalRecordCount);
            var openSource = new Res_ContentLogic().GetCourseOpenResource(BaseUtility.SiteOrganizationID, CourseID);
            foreach (var row in dtResource)
            {
